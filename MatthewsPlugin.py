...
class MatthewsPlugin:
    ...
    def output(self, outputfile):
        ...
        for i in range(len(other_labels)):
         ...
         all_matthews.append([self.parameters["label"]]+matthews)

        matthews_df = pd.DataFrame(all_matthews, columns=['Method']+list(range(1,11)))
        matthews_df.to_csv(outputfile, index=False)
        matthews_df
    ...
    #    scores_masif = pd.read_csv(SCORES_MASIF)
    #    all_matthews = []
    #    matthews, matthews, metrics = find_optimal_matthew(scores_masif, 'score', 'label')
    #    all_matthews.append([self.parameters["label"]]+matthews)
